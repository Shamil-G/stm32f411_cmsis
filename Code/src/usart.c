...
uint8_t usart1_dma_rx(uint8_t* rxData, uint16_t buff_size, uint32_t timeout){
	...
	// При выключении &= ~DMA_SxCR_EN - сразу срабатвает флаг DMA_LISR_TCIF2
	// DMA2_Stream2->CR &= ~DMA_SxCR_EN;
	// while ((DMA2_Stream2->CR & DMA_SxCR_EN) && (usart_ticks < timeout));
	// При внесении адреса в DMA2_Stream2->M0AR - USART сразу переходит в DISABLE
...
//----------------------------------------------------------------------------
uint8_t usart1_rx(uint8_t * txData, uint16_t buff_size, uint32_t timeout){
	while( (usart_ticks < timeout) &&
		   (USART1->SR & USART_SR_RXNE)) {
		usart_rx_ovr = (usart_counter>=USART_SIZE_BUFFER)?1:0;
		uint8_t pos = (usart_counter>=USART_SIZE_BUFFER)?0:usart_counter;
		usart_buffer[pos] = USART1->DR;
		usart_counter++;
	}
}
...
#ifdef USE_DMA_IRQ
	while(tx_ready==0 && usart_ticks < timeout);
	if( usart_ticks >= timeout ) return 0;
#endif
